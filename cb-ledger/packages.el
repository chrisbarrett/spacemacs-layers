    ...
    (progn
      ...
      (setq ledger-reports
            '(("assets" "ledger -f %(ledger-file) bal assets")
              ...
              ("spending review (reg)" "ledger -f %(ledger-file) reg expenses --sort total -p 'last 7 days' --invert --by-payee")
              ("spending review (account)" "ledger -f %(ledger-file) reg %(account) --sort total -p 'last 7 days' --invert --by-payee")
              ("this week" "ledger -f %(ledger-file) %(report-type) checking -p 'this week' --invert")
              ("this month" "ledger -f %(ledger-file) %(report-type) checking -p 'this month' --invert")
              ("since payday" "ledger -f %(ledger-file) %(report-type) checking -b %(last-payday) --invert")
              ("previous pay period" "ledger -f %(ledger-file) %(report-type) checking -p %(prev-pay-period) --invert"))))))
